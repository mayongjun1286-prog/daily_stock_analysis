...
const settingsHelpZhCN: SettingsHelpMap = {
  ...
  'settings.system.market_review': {
    ...
    usage: 'MARKET_REVIEW_ENABLED 开启大盘分析；DAILY_MARKET_CONTEXT_ENABLED 默认开启，会把当日大盘摘要用于个股分析 Prompt 与保守护栏；MARKET_REVIEW_REGION 支持输入逗号分隔的市场子集（如 us,jp 或 us,jp,kr），不传可回退为 us；MARKET_REVIEW_COLOR_SCHEME 选择配色。',
    valueNotes: [
      'us 覆盖美股，jp 覆盖日股，kr 覆盖韩股，both 覆盖全部（us,jp,kr）。',
      'MARKET_REVIEW_REGION 直接写入文本框，支持逗号分隔的子集；空值或非法值会回退到 us。',
...
const settingsHelpEnUS: SettingsHelpMap = {
  ...
  'settings.system.market_review': {
    ...
    usage: 'MARKET_REVIEW_ENABLED toggles market review; DAILY_MARKET_CONTEXT_ENABLED is on by default and controls whether the daily market summary is injected into stock-analysis prompts and conservative guardrails; MARKET_REVIEW_REGION accepts a comma-separated region subset (for example us,jp or us,jp,kr), and `both` keeps all supported markets; invalid or empty values fall back to `us`; MARKET_REVIEW_COLOR_SCHEME selects colors.',
    valueNotes: [
      'us covers US stocks, jp covers Japan, kr covers Korea, and both covers all (us,jp,kr).',
      'MARKET_REVIEW_REGION is free-text input; you can enter a comma-separated subset like us,jp or us,jp,kr.',
